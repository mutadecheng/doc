...
select '' fieldSeparator, '' rowSeparator,'
/*G'+'O*/'+' 
G'+'O'+'
' tableSeparator,'1.建表.sql' [fileName];
...
select [col_id],[col_name],col_typename,col_len,col_identity,col_seed,col_increment,collation   ,col_null          ,[text] col_DefaultValue,col_dridefname [ConstraintName_DefaultValue]  
...
--（.2.3）主码 和 唯一 约束 。 CONSTRAINT_TYPE：  'PRIMARY KEY' 和 'UNIQUE'
...
select ' create table [dbo].['+@tbName+'] ( ';
...
-- [类型] (长度)
+(case when(0!=charindex('char',col_typename)) then ' ('+convert(varchar(100),[col_len])+')' else '' end)  
...
--(.3.3) unique 约束              ALTER TABLE table_a ADD unique(aID);
select ('
alter table '+ quotename(@tbName)
